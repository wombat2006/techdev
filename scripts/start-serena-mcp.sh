#!/bin/bash
# Start Serena MCP with single instance guarantee
...
if [[ -f "$LOCK_FILE" ]]; then
    ...
    if [[ -n "$OLD_PID" ]] && ! kill -0 "$OLD_PID" 2>/dev/null; then
        echo "[SERENA-MCP] Removing stale lock file (PID $OLD_PID not running)" >&2
        rm -f "$LOCK_FILE"
    fi
fi
...
if ! flock -n 200; then
    ...
    OLD_PID=$(cat "$LOCK_FILE" 2>/dev/null || echo "")
    if [[ -n "$OLD_PID" ]] && kill -0 "$OLD_PID" 2>/dev/null; then
        echo "[SERENA-MCP] Killing existing instance (PID: $OLD_PID)" >&2
        # Kill process tree
        ...
        echo "[SERENA-MCP] Removing stale lock file" >&2
        rm -f "$LOCK_FILE"
    fi
    ...
    if ! flock -w 5 200; then
        echo "[SERENA-MCP] Failed to acquire lock after cleanup" >&2
        exit 1
    fi
fi
...
if [[ "$WRITTEN_PID" != "$$" ]]; then
    echo "[SERENA-MCP] ERROR: Failed to write PID to lock file (wrote: '$WRITTEN_PID', expected: $$)" >&2
    rm -f "$LOCK_FILE"
    exit 1
fi

echo "[SERENA-MCP] Lock file created successfully with PID $$" >&2

# Kill any orphaned serena MCP processes
pkill -f 'serena start-mcp-server' 2>/dev/null || true
sleep 0.5

# Start Serena MCP server (stdio will be inherited)
# Using exec replaces this process, so our PID remains valid in lock file
exec uv run serena start-mcp-server
