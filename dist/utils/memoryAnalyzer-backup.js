...
class MemoryAnalyzer {
    ...
    getDetailedMemoryStats() {
        ...
        const heapSpaces = v8_1.default.getHeapSpaceStatistics();
        const oldSpace = heapSpaces.find(space => space.space_name === 'old_space');
        const processMemory = process.memoryUsage();
        return {
            timestamp: Date.now(),
            elapsed: Math.round(perf_hooks_1.performance.now() - this.startTime),
            process: {
     ...
     */
    recordMeasurement(label, metadata) {
     ...
     */
    compareMeasurements(fromLabel, toLabel) {
    ...
    calculateTrend(metric, values, timestamps) {
    ...
    async forceGarbageCollection() {
        ...
        const gcStartTime = perf_hooks_1.performance.now();
        global.gc();
        const gcTime = Math.round(perf_hooks_1.performance.now() - gcStartTime);
...
exports.default = MemoryAnalyzer;
//# sourceMappingURL=memoryAnalyzer-backup.js.map