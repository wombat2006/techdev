...
export type TaskType = 'architecture' | 'code-review' | 'implementation' | 'security' | 'optimization' | 'integration' | 'general';
/**
...
export interface IWallBounceProviderManager {
    ...
    readonly circuitBreakers: Map<string, any>;
    /**
     * プロバイダーを初期化
     */
...
export interface IWallBounceInvoker {
     ...
     * GPT-5を呼び出し
     */
    invokeGPT5(prompt: string, config: {
        model: string;
     ...
     * 汎用プロバイダー呼び出し
     */
    invokeProvider(providerKey: string, prompt: string, options: ExecuteOptions): Promise<LLMResponse>;
}
/**
...
export interface IWallBouncePromptBuilder {
     ...
     * プロバイダー別プロンプトを構築
     */
    buildProviderPrompt(prompt: string, providerKey: string, options: ExecuteOptions): string;
    /**
     * Deep sequentialプロンプトを構築
     */
    buildDeepSequentialPrompt(prompt: string, providerKey: string, roundNumber: number, previousResponses: LLMResponse[]): string;
    /**
     * Aggregatorプロンプトを構築
     */
    buildAggregatorPrompt(prompt: string, responses: LLMResponse[]): string;
    /**
     * 累積コンテキストを構築
     */
     ...
     * Wall-Bounce結果を構築
     */
    buildWallBounceResult(finalAnalysis: LLMResponse, responses: LLMResponse[], options: ExecuteOptions, startTime: number): WallBounceResult;
}
/**
