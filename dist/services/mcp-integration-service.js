...
exports.mcpIntegrationService = exports.MCPIntegrationService = void 0;
const logger_1 = require("../utils/logger");
const mcp_config_manager_1 = require("./mcp-config-manager");
const mcp_approval_manager_1 = require("./mcp-approval-manager");
class MCPIntegrationService {
    executionHistory = [];
    /**
     * Execute MCP tools with full enterprise governance
     */
    async executeMCPTools(openaiClient, request) {
        const startTime = Date.now();
        logger_1.logger.info('🚀 MCP Integration Service - Execution Started', {
        ...
        try {
            ...
            const optimizedTools = mcp_config_manager_1.mcpConfigManager.getOptimizedToolsForContext(request.context);
            const costEstimate = mcp_config_manager_1.mcpConfigManager.estimateToolCosts(optimizedTools, 5);
            ...
            if (costEstimate.budget_warning) {
                ...
                logger_1.logger.warn('💰 Budget Warning', { warning: costEstimate.budget_warning });
            }
            // Phase 2: Pre-execution Approval Workflow  
            const approvalStartTime = Date.now();
            const approvalResults = await this.processApprovalWorkflow(optimizedTools, request.context, request.requestedBy || 'system');
            ...
            if (request.dryRun) {
                logger_1.logger.info('🧪 Dry Run Mode - Simulating execution', {
            ...
            const executionResult = await this.executeApprovedTools(openaiClient, approvalResults.approvedTools, request.context);
            ...
            result.success = true;
            logger_1.logger.info('✅ MCP Integration Service - Execution Completed', {
                success: true,
            ...
            logger_1.logger.error('❌ MCP Integration Service - Execution Failed', {
                error: result.error,
                context: request.context
            });
        }
    ...
    async processApprovalWorkflow(tools, context, requestedBy) {
        const summary = {
            total_requests: 0,
            auto_approved: 0,
            manual_approved: 0,
        ...
        for (const tool of tools) {
            for (const allowedOperation of tool.allowed_tools) {
                ...
                const approvalRequest = await mcp_approval_manager_1.mcpApprovalManager.requestApproval(tool.server_label, allowedOperation, { tool_config: tool }, context, requestedBy);
                if (!approvalRequest.requiresApproval) {
                    ...
                    const shouldApprove = this.simulateApprovalDecision(tool.server_label, allowedOperation, context);
                    if (shouldApprove) {
        ...
        logger_1.logger.info('📋 Approval workflow completed', {
    ...
    simulateApprovalDecision(toolName, operation, context) {
    ...
    async executeApprovedTools(openaiClient, approvedTools, context) {
        const results = [];
        const mcp_calls = [];
        const tool_breakdown = [];
        const tool_response_times = {};
        let actual_cost = 0;
        ...
        Use tools efficiently and provide comprehensive results.`,
            ...
            if (response.output) {
                const mcpCallItems = response.output.filter((item) => item.type === 'mcp_call');
     ...
     */
    getAnalytics(timeWindowMs = 24 * 60 * 60 * 1000) {
     ...
     */
    getSystemStatus() {
        const configStatus = mcp_config_manager_1.mcpConfigManager.getConfigurationStatus();
        const approvalStats = mcp_approval_manager_1.mcpApprovalManager.getApprovalStatistics();
        const recentExecutions = this.executionHistory.filter(exec => exec.timestamp >= Date.now() - (60 * 60 * 1000) // Last hour
        ).length;
        // Health assessment
...
// Singleton instance
exports.mcpIntegrationService = new MCPIntegrationService();
exports.default = exports.mcpIntegrationService;
