...
exports.mcpApprovalManager = exports.MCPApprovalManager = void 0;
...
class MCPApprovalManager {
    ...
    initializeApprovalPolicies() {
        ...
        logger_1.logger.info('🔐 MCP Approval Manager initialized', {
     ...
     */
    async requestApproval(toolName, operation, operationArgs, context, requestedBy = 'system') {
        // Check if approval is needed based on configuration
        const approvalRequirement = mcp_config_manager_1.mcpConfigManager.getApprovalRequirement(toolName, operation, context);
        if (approvalRequirement === 'never') {
            logger_1.logger.debug('✅ Auto-approved: No approval required', { toolName, operation });
        ...
        if (policy?.auto_approve_conditions) {
            for (const condition of policy.auto_approve_conditions) {
                if (condition.condition(request)) {
                    logger_1.logger.info('✅ Auto-approved by policy', {
                        requestId,
                        toolName,
                        operation,
     ...
     */
    async processApproval(requestId, approved, approverRole, approverIdentity, rejectionReason, conditions) {
        ...
        logger_1.logger.info(approved ? '✅ Request approved' : '❌ Request rejected', {
            requestId,
     ...
     */
    async isOperationApproved(requestId) {
        ...
        if (response.expires_at && Date.now() > response.expires_at) {
            logger_1.logger.warn('⏰ Approval expired', { requestId });
            return { approved: false, expired: true };
        }
        return { approved: response.approved, response };
    }
    assessRiskLevel(toolName, operation, args, context) {
        // Critical operations
    ...
    recordApprovalDecision(requestId, response) {
    ...
    handleApprovalTimeout(requestId) {
        ...
        logger_1.logger.warn('⏰ Approval request timed out', {
            requestId,
            tool: request.tool_name,
            operation: request.operation,
     ...
     */
    getApprovalStatistics(timeWindow = 24 * 60 * 60 * 1000) {
     ...
     */
    exportAuditLog(startTime, endTime) {
...
// Singleton instance
exports.mcpApprovalManager = new MCPApprovalManager();
exports.default = exports.mcpApprovalManager;
//# sourceMappingURL=mcp-approval-manager.js.map