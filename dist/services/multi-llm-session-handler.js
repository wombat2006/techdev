...
exports.getMultiLLMSessionHandler = exports.MultiLLMSessionHandler = void 0;
const wall_bounce_analyzer_1 = require("./wall-bounce-analyzer");
const codex_mcp_wrapper_1 = require("./codex-mcp-wrapper");
const codex_session_manager_1 = require("./codex-session-manager");
const logger_1 = require("../utils/logger");
class MultiLLMSessionHandler {
    wallBounceAnalyzer;
    codexWrapper;
    sessionManager;
    ...
    constructor() {
        this.wallBounceAnalyzer = new wall_bounce_analyzer_1.WallBounceAnalyzer();
        this.codexWrapper = new codex_mcp_wrapper_1.CodexMCPWrapper();
    ...
    async continueSessionWithWallBounce(args) {
        ...
        try {
            ...
            logger_1.logger.info(`🔄 Multi-LLM routing for turn ${turnNumber}`, {
            ...
            logger_1.logger.error('MultiLLMSessionHandler error:', error);
            return this.createErrorResponse(args.sessionId, '', error instanceof Error ? error.message : 'Unknown error', startTime);
        }
    }
    /**
     * ターン1: 直接Codex実行
     */
    async executeDirectCodex(args, session, startTime) {
        logger_1.logger.info('🎯 Turn 1: Direct Codex execution');
     ...
     * ターン2以降: Wall-Bounce分析実行
     */
    async executeWallBounceAnalysis(args, session, turnNumber, startTime) {
        logger_1.logger.info(`🏓 Turn ${turnNumber}: Wall-Bounce multi-LLM analysis`);
        // 会話履歴を取得してコンテキスト構築
     ...
     * 文脈プロンプトの構築
     */
    buildContextualPrompt(currentPrompt, conversationHistory, turnNumber) {
...
`.trim();
     ...
     */
    getWallBounceConfig(turnNumber) {
        if (turnNumber === 2) {
     ...
     * エラーレスポンス生成
     */
    createErrorResponse(sessionId, conversationId, error, startTime) {
        return {
            success: false,
            sessionId,
            conversationId,
...
exports.MultiLLMSessionHandler = MultiLLMSessionHandler;
const getMultiLLMSessionHandler = () => {
    return new MultiLLMSessionHandler();
};
exports.getMultiLLMSessionHandler = getMultiLLMSessionHandler;
//# sourceMappingURL=multi-llm-session-handler.js.map