...
| B5 | プロンプト解析が regex 主体 | 日本語・専門語 routing 精度低 → **Phase 0 で形態素解析を導入** |
| B6 | 独自 DB 未整備 | 社内正本・版管理不可 |
| B7 | APS（e-Gov / NDL / 判例）未実装 | 法令・文献捏造リスク |
| B8 | `getProviderOrder(taskType)` 固定 | 子タスク最適化不可 |
| B9 | モデル / effort / CoT / temperature が統一スキーマなし | 子タスクごとの推論制御不可 → **InferenceProfile（TS-20）** |
...
| Phase | 期間目安 | 内容 |
|---|---|---|
| **0** | 1〜1.5 ヶ月 | Hard gate、PromptAnalyzer（**形態素解析**）、辞書 v0、confidence 固定値廃止、**憲法ラウンド enforce**、**InferenceProfile（TS-20）**、Orchestrator 統合 |
...
| `llm_analyze` | Antigravity CLI（Gemini 2.5 Pro）、Sonnet |
...
interface ChildTask {
  ...
  /** To-Be: preset or explicit InferenceProfile — see §10 */
  inferenceProfile?: InferenceProfile | 'fast' | 'balanced' | 'deep' | 'critical';
}
```
...
**方針**: 日本語クエリ・専門語の routing 精度向上のため、**形態素解析を行う**（Phase 0 / PromptAnalyzer）。現状の regex 主体解析（B5）を置き換える。

### 実施内容

| 項目 | 内容 |
|------|------|
| **タイミング** | ユーザープロンプト受信後、RAG / Grounding クエリ生成 **前**（1 リクエスト 1 回 parse） |
| **用途** | クエリ正規化、専門辞書との照合、TaskRouter 入力特徴量、hybrid search 用 term 抽出 |
| **consumer** | PromptAnalyzer → GroundingOrchestrator / TaskRouter / 辞書 v0（parse 結果を共有） |
| **解析器** | 形態素解析エンジン（MeCab 等、具体製品は Phase 0 実装時に選定） |

### 採用 / 非採用

- **採用**: PromptAnalyzer、RAG hybrid search の term 抽出（条件付き）
- **非推奨**: Embedding 前処理への一律適用、consensus 計算単体、プロンプト本体の形態素置換（意味改変リスク）

### AS-IS → To-Be

| | AS-IS | To-Be |
|---|--------|--------|
| プロンプト解析 | regex 主体 | **形態素解析 + 辞書** |
| 日本語 routing | 精度低（B5） | 専門語・略語展開を改善 |
...
## 10. Inference Profiles（モデル・effort・CoT・temperature）

**方針**: 子タスクおよび provider 呼び出しごとに **InferenceProfile** で推論パラメータを統一制御する（TS-20）。Claude Code / Codex / `agy` は **同列の peer provider**；Opus は **aggregator 専用**。
...
interface InferenceProfile {
  ...
  cot?: 'off' | 'brief' | 'full';  // Chain-Of-Thought — effort と独立
}
```

| パラメータ | 制御対象 |
|------------|----------|
| `model` | Haiku / Sonnet / Opus、GPT-5 Codex、Gemini Pro / Flash 等 |
| `effort` | Claude `--effort`、Codex `reasoning_effort` |
| `cot` | CoT プロンプト方針と出力露出（`off` = 最終回答のみ、`full` = 段階的推論） |
| `temperature` | サンプリング（コード・法令・合成は低め） |

**CoT と SSE thinking**: UI の thinking ストリームは `cot !== 'off'` 時に有効化。`effort` の代替ではない。

### 10.2 Preset

| Preset | 用途 |
|--------|------|
| `fast` | Haiku / Flash、低 effort、`cot: off` |
| `balanced` | Sonnet / gpt-5、中 effort、`cot: brief` |
| `deep` | Sonnet / Pro、高 effort、`cot: full` |
| `critical` | Opus aggregator、max effort、`cot: brief`（合成のブレ抑制） |

TaskRouter が `ChildTaskKind` から preset を選択。`llm_aggregate` は `critical` 固定。

### 10.3 Phase 0 実装範囲

- `config/inference-profiles.json`
- Claude MCP / Codex MCP への profile pass-through
- Wall-Bounce API `profile` / `inference` オプション（To-Be）

詳細: [TECH_STACK_INFERENCE_PROFILES.md](./TECH_STACK_INFERENCE_PROFILES.md)、[WALL_BOUNCE_SYSTEM.md § Inference Profiles](../WALL_BOUNCE_SYSTEM.md#inference-profiles-model-effort-cot-temperature)
...
- InferenceProfile: [`./TECH_STACK_INFERENCE_PROFILES.md`](./TECH_STACK_INFERENCE_PROFILES.md)
